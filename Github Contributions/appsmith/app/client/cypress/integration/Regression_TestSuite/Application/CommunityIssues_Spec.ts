...
function cyWaitResponseFunction(interception: any){
    agHelper.Sleep();
...
function propPaneResponseFunction($selectedRow: any) {
...
function validateHeaderFunction() {
  homePage.NavigateToHome();
  homePage.ImportApp("CommunityIssuesExport.json");
  cy.wait("@importNewApplication").then(cyWaitResponseFunction);
...
function validateTableDisabledRowFunction() {
...
function verifyDefaultSearchText() {
  ee.SelectEntityByName("Table1", "Widgets");
...
function describeCallBackFunction() {
  before(clearLocalStorage);
  beforeEach(restoreLocalStorage);
  afterEach(saveLocalStorage);

 
  it("1. Import application json and validate headers", validateHeaderFunction);

  it("2. Validate table navigation with Server Side pagination enabled with Default selected row", validateTableEnabledRowFunction);

  it("3. Validate table navigation with Server Side pagination disabled with Default selected row selection", validateTableDisabledRowFunction);

  it("4. Change Default selected row in table and verify", changeRow);

  it.skip("5. Verify Default search text in table as per 'Default Search Text' property set + Bug 12228", verifyDefaultSearchText);

  it.skip("6. Validate Search table with Client Side Search enabled & disabled", validateSearchTable);

  it("7. Validate Filter table", validateFilterTable);
...
describe("AForce - Community Issues page validations", describeCallBackFunction);
